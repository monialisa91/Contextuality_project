...
for j in range(0,4):
	...
	for i in range(0,100000):
		...
		czysty = np.trace(mulmul(Density, Density))
		Purity1.append(czysty)
	Evolution.append(Evolution1)
	Purity.append(Purity1)
...
ax1.set_ylim(-4, 1)
ax1.legend(loc = (0.75,0.7), frameon = True, fancybox = True, edgecolor = 'black')
ax1.set_xlabel("$\mathbf{t}$")
ax1.set_ylabel("$\mathbf{\Delta}(t)$")


left, bottom, width, height = [0.25, 0.64, 0.2, 0.2]
ax2 = fig.add_axes([left, bottom, width, height])

ax2.plot(Czas[:8000],Purity[0][:8000], linewidth = 2, label = "$\mathbf{V_{CC}}=0$")
ax2.plot(Czas[:8000],Purity[1][:8000],linewidth = 2,label = "$\mathbf{V_{CC}}=0.1$")
ax2.plot(Czas[:8000],Purity[3][:8000],linewidth = 2,label = "$\mathbf{V_{CC}}=0.3$")
ax2.set_xlabel("$\mathbf{t}$")
ax2.set_ylabel("$\mathbf{\mathcal{P}}(t)$")
