...
ax.plot(Czas, Evolution[0], linewidth = 2, label = "$\mathbf{c}=0$")
ax.plot(Czas,Evolution[5],'y-',linewidth = 2,label = "$\mathbf{c}=0.5$")
ax.plot(Czas,Evolution[10],'r-',linewidth = 2,label="$\mathbf{c}=1$")
ax.plot(Czas,Evolution[15],'m-',linewidth=2,label="$\mathbf{c}=1.5$")
ax.plot(Czas,Evolution[20],'g-',linewidth=2,label="$\mathbf{c}=2$")
ax.axhline(y = -3, color = 'black', linewidth = 3)
ax.set_xlabel('$\mathbf{t}$')
ax.set_ylabel(r'$\mathbf{\Delta}(t)$', fontsize = 11, va = 'baseline', labelpad = 10)
ax.legend(loc = (0.75,0.1), frameon = True, fancybox = True, edgecolor = 'black')
left, bottom, width, height = [0.25, 0.55, 0.3, 0.3]

ax2 = fig.add_axes([left, bottom, width, height])
ax2.plot(Czas[:limit], Evolution[0][:limit], linewidth = 2, label = "$\mathbf{c}=0$")
ax2.plot(Czas[:limit],Evolution[5][:limit],'y-',linewidth = 2,label = "$\mathbf{c}=0.5$")
ax2.plot(Czas[:limit],Evolution[10][:limit],'r-',linewidth = 2,label="$\mathbf{c}=1$")
ax2.plot(Czas[:limit],Evolution[15][:limit],'m-',linewidth=2,label="$\mathbf{c}=1.5$")
ax2.plot(Czas[:limit],Evolution[20][:limit],'g-',linewidth=2,label="$\mathbf{c}=2$")
#ax2.axhline(y = -3, color = 'black', linewidth = 3)
plt.show()		
