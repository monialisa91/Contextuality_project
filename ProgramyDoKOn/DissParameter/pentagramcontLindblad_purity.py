...
ax.plot(Czas[:limit], Evolution[0][:limit], linewidth = 2, label = "$\mathbf{c}=0$")
ax.plot(Czas[:limit],Evolution[5][:limit],'y-',linewidth = 2,label = "$\mathbf{c}=0.5$")
ax.plot(Czas[:limit],Evolution[10][:limit],'r-',linewidth = 2,label="$\mathbf{c}=1$")
ax.plot(Czas[:limit],Evolution[15][:limit],'m-',linewidth=2,label="$\mathbf{c}=1.5$")
ax.plot(Czas[:limit],Evolution[20][:limit],'g-',linewidth=2,label="$\mathbf{c}=2$")
ax.set_xlabel('$\mathbf{t}$')
ax.set_ylabel(r'$\mathbf{\rho^{2}}(t)$', fontsize = 11, va = 'baseline', labelpad = 10)
plt.show()		
